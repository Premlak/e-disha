...
const generatePDF = () => {
  ...
  useEffect(() => {
    ...
    if (selectedUser) {
        result = result.filter(product => product.user === selectedUser);
      }
    ...
    setCurrentPage(1);
  }, [products, filters, sortConfig, dateRange, issueStatus, selectedUser]);
  useEffect(() => {
    fetchInitialData();
  }, []);
        ...
        {product.issued ? (
          ...
          <>
            <button
              ...
              title={!selectedUser ? "Please select a user first" : ""}
            >
              Issue
            </button>
            <button
